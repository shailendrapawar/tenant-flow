...
export const SearchRoomsQuerySchema = z.object({
    ...
    occupancyStatus: z.enum(["vaccant", "occupied"]).optional(),
...
export const UpdateRoomPayloadSchema = z.object({
    roomNumber: z.string().optional().openapi({ example: '101' }),
    floor: z.number().optional().openapi({ example: 1 }),
    roomRent: z.number().optional().openapi({ example: 10000 }),
    capacity: z.number().openapi({ example: 2 }),
    occupancyCount: z.number().optional(),
    notes: z.string().optional().openapi({ example: 'Notes for room' }),
...
export type UpdateRoomPayloadType = z.infer<typeof UpdateRoomPayloadSchema> 